...
test('switch to summer time', async ({ page }) => {
  ...
  await expect(page.getByTestId('electricity-price-chart')).toBeVisible()
  await expect(page.getByTestId('bar-segment-price')).toHaveCount(19)
  ...
  await expect(page.getByTestId('bar-total-price').nth(14)).toHaveText('14.78')
...
test('return to standard time', async ({ page }) => {
  ...
  await expect(page.getByTestId('bar-total-price').nth(14)).toHaveText('21.08')
