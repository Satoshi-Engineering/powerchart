...
test('price chart', async ({ page }) => {
  ...
  await expect(page.getByTestId('bar-total-price').nth(14)).toHaveText('11.88')
...
test('switch to summer time', async ({ page }) => {
  ...
  await expect(page.getByTestId('bar-total-price').nth(14)).toHaveText('8.54')
...
test('return to standard time', async ({ page }) => {
  ...
  await expect(page.getByTestId('bar-total-price').nth(14)).toHaveText('14.84')
